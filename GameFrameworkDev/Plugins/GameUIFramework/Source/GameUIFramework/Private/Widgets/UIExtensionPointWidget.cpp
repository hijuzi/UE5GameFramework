...
TSharedRef<SWidget> UUIExtensionPointWidget::RebuildWidget()
{
	if (!IsDesignTime() && ExtensionPointTag.IsValid())
	{
		...
		if (UGameCoreLocalPlayer* LocalPlayer = Cast<UGameCoreLocalPlayer>(GetOwningLocalPlayer()))
		{
			LocalPlayer->CallAndRegister_OnPlayerStateSet(
				UGameCoreLocalPlayer::FPlayerStateSetDelegate::FDelegate::CreateUObject(this, &UUIExtensionPointWidget::RegisterExtensionPointForPlayerState)
			);
		}
	}
...
void UUIExtensionPointWidget::RegisterExtensionPointForPlayerState(UGameCoreLocalPlayer* LocalPlayer, APlayerState* PlayerState)
{
	if (UUIExtensionSubsystem* ExtensionSubsystem = GetWorld()->GetSubsystem<UUIExtensionSubsystem>())
	{
		TArray<UClass*> AllowedDataClasses;
